# T016 — Developer commands documentation
...
## Exact Allowed Files
Only process/tasks/T016.md implementation targets, their focused tests, and stale documentation named below. No source code is authorized by this planning card.
...
docs/DEVELOPER_GUIDE.md;docs/TROUBLESHOOTING.md;scripts/smoke_test.sh;compose.yaml;.github/workflows/test.yml

## Task-specific focused command
uv run pytest; uv run ruff format --check; uv run ruff check; uv run pyrefly check; uv run pymarkdown scan README.md docs/

## Task-specific model
Implementation: LOW_COST

## Task-specific allowed files
docs/DEVELOPER_GUIDE.md, docs/TROUBLESHOOTING.md, process/PROGRESS_LOG.md

## Task-specific constraint
Document only commands actually verified; flag scripts/smoke_test.sh down -v.
