# T015 — Migration architecture verification
...
## Exact Allowed Files
Only process/tasks/T015.md implementation targets, their focused tests, and stale documentation named below. No source code is authorized by this planning card.
...
src/memory/postgres.py;src/memory/__init__.py;tests/smoke/test_persistence.py

## Task-specific focused command
uv run pytest tests/smoke/test_persistence.py -v --run-docker (or record Docker unavailable)

## Task-specific model
Implementation: STRONG_REVIEW_REQUIRED

## Task-specific allowed files
process/DECISION_LOG.md, docs/ARCHITECTURE.md, docs/DEVELOPER_GUIDE.md, process/PROGRESS_LOG.md

## Task-specific constraint
Documentation/ADR only; do not add Alembic, ORM, or tables.
