...
## Exact Allowed Files
Only process/tasks/T013.md implementation targets, their focused tests, and stale documentation named below. No source code is authorized by this planning card.
...
src/service/service.py;src/service/utils.py;tests/service/test_service.py

## Task-specific focused command
uv run pytest tests/service/test_service.py -q, then uv run pytest

## Task-specific model
Implementation: LOW_COST

## Task-specific allowed files
src/service/service.py, src/service/utils.py, tests/service/test_service.py, docs/ARCHITECTURE.md, process/PROGRESS_LOG.md

## Task-specific constraint
Middleware only; no Task/trace IDs.
