...
This document records the Phase 2 identity architecture and the T021–T023 schema, the Phase 3 T031/T032/T034 Task domain, T081/T082 Approval persistence and decision services, and the Phase 7 planning contract. ADR-004, ADR-005, accepted ADR-008, and proposed ADR-009 are authoritative for their respective domains.
...
## Phase 7 observability persistence planning (ADR-009)

T091 and T092 add only the normalized AgentRun and ToolCall evidence described
by ADR-009. AgentRun owns a required `task_run_id`; ToolCall owns a required
`agent_run_id`. Tenant ownership is always resolved through
...
ToolCall uniqueness is `(agent_run_id, call_index)`. Step position is
zero-based because TaskStep persistence remains deferred. All timing is UTC;
duration is a nullable bounded integer in milliseconds; errors, provider
metadata, arguments, and results are bounded and sanitized. T092 owns
